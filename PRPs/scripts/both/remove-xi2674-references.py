#!/usr/bin/env python3
"""
Remove XI2674 Missing Photo References - Phase 6B.2 Script

This script removes XI2674.htm missing photo references from HTML thumbnail galleries.
These entries represent missing photo pages that were never created.

Problem: References to XI2674.htm create broken links in thumbnail galleries
Solution: Remove the entire thumbnail entry (image + map + separator) while preserving gallery layout

Usage:
    python3 remove-xi2674-references.py --site=htm --dry-run    # Preview HTM site
    python3 remove-xi2674-references.py --site=new --dry-run    # Preview NEW site
    python3 remove-xi2674-references.py --site=both --dry-run   # Preview both sites
    python3 remove-xi2674-references.py --site=htm             # Clean HTM site
    python3 remove-xi2674-references.py --site=new            # Clean NEW site
    python3 remove-xi2674-references.py --site=both           # Clean both sites

Features:
- Removes complete XI2674 thumbnail entries (img + map + separator)
...
def get_html_files_with_xi2674(base_path: str) -> List[Path]:
    """Get list of HTML files that reference XI2674."""
    base = Path(base_path)
    html_files = []
...
def remove_xi2674_entries(content: str) -> Tuple[str, int]:
    """
    Remove XI2674.htm thumbnail entries from HTML content.

    Each XI2674 entry consists of:
    1. <img src="..." usemap="#Map2674" ...>
    2. <map name="Map2674" id="Map2674">...</map>
    ...
    # Find and remove each XI2674 entry individually
    removal_count = 0
    working_content = content

    # Pattern to find img tags that use Map2674 and extract the map ID
    img_pattern = r'<img\s+[^>]*usemap="#Map2674"[^>]*>'

    # Find all XI2674 images
    ...
    for img_match in reversed(img_matches):  # Process in reverse to maintain indices
        ...
        map_pattern = r'<map\s+name="Map2674"[^>]*>.*?</map>'
...
def process_file(file_path: Path, dry_run: bool = False) -> Tuple[bool, str, int]:
    """
    Process a single HTML file to remove XI2674 references.
    ...
    try:
        ...
        # Remove XI2674 entries
        modified_content, removal_count = remove_xi2674_entries(original_content)

        if removal_count == 0:
            return True, f"No XI2674 references found in {file_path}", 0

        if dry_run:
            return True, f"Would remove {removal_count} XI2674 entries from {file_path}", removal_count

        # Write modified content
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(modified_content)

        return True, f"✅ Removed {removal_count} XI2674 entries from {file_path}", removal_count
...
def process_site(site_dir: str, dry_run: bool = False) -> Dict[str, Any]:
    ...
    html_files = get_html_files_with_xi2674(site_dir)

    if not html_files:
        return {
            'success': True,
            'message': f"No files with XI2674 references found in {site_dir}",
            'files_processed': 0,
            'files_successful': 0,
            'files_failed': 0,
            'total_removals': 0
    ...
    logger.info(f"Found {len(html_files)} files with XI2674 references in {site_dir}")
    ...
    if not dry_run:
        ...
        logger.info(f"🔍 Dry-run for {site_dir}: {action} {total_removals} XI2674 references from {len(html_files)} files")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Remove XI2674.htm references from HTML thumbnail galleries'
    )
    parser.add_argument(
        '--site',
    ...
    logger.info(f"  XI2674 entries {action.lower()}: {total_removals}")

    if args.dry_run:
        logger.info(f"\n🔍 This was a dry-run. To actually remove XI2674 references, run without --dry-run")
    elif overall_success:
        logger.info(f"\n✅ All XI2674 references cleaned successfully!")
        logger.info(f"📝 Next steps:")
        logger.info(f"  1. Review the cleaned files in your browser")
        logger.info(f"  2. Commit changes: git add . && git commit -m 'Remove XI2674.htm references - cleaned {total_removals} broken thumbnail entries'")
    else:
