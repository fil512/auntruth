# Modernize Genealogy Site Task: modernize-java-applets
...
## Current Issue
Modernize Java applet sound players to work in modern browsers
...
1. **Search ALL subdirectories recursively** - Use Grep with count mode: `.au` in `docs/htm`
...
grep -r ".au" docs/htm --include="*.htm" --include="*.html" -l | wc -l

# Count total occurrences
grep -r ".au" docs/htm --include="*.htm" --include="*.html" -c

# List affected files for verification
grep -r ".au" docs/htm --include="*.htm" --include="*.html" -l
```
...
- [ ] I have verified I'm working in the correct git branch: `task-004-modernize-java-applets`
...
1. **Verify setup**: Confirm you're in branch `task-004-modernize-java-applets` and tracking shows "IN PROGRESS"
...
- Replace Java applet sound players with modern HTML5 audio elements
- Preserve all .au audio files - DO NOT DELETE THEM
- Use modern web standards (UTF-8, proper DOCTYPE, etc.)
- Keep the genealogical content and structure intact
- Ensure audio files are accessible and playable in modern browsers

## Success Criteria
- All Java applet sound players are replaced with modern HTML5 audio elements
- All .au audio files remain accessible and functional
- Audio controls work in modern browsers (Chrome, Firefox, Safari, Edge)
