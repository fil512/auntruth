# Modernize Genealogy Site Task: fix-auntruuth-paths
...
**Description:** Convert /AuntRuth/ references to work from /auntruth/ base
...
1. **Search ALL subdirectories recursively** - Use Grep with count mode: `/AuntRuth/` in `docs/htm`
...
grep -r "/AuntRuth/" docs/htm --include="*.htm" --include="*.html" -l | wc -l

# Count total occurrences
grep -r "/AuntRuth/" docs/htm --include="*.htm" --include="*.html" -c

# List affected files for verification
grep -r "/AuntRuth/" docs/htm --include="*.htm" --include="*.html" -l
```
...
- [ ] I have verified I'm working in the correct git branch: `task-002-fix-auntruuth-paths`
...
1. **Verify setup**: Confirm you're in branch `task-002-fix-auntruuth-paths` and tracking shows "IN PROGRESS"
...
## Success Criteria
- All instances of `/AuntRuth/` are properly addressed
