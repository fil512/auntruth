# Modernize Genealogy Site Task: add-doctype
...
**Item:** 011
**Task:** add-doctype
**Description:** Add proper DOCTYPE declarations
...
1. **Search ALL subdirectories recursively** - Use Grep with count mode: `<!DOCTYPE` in `docs/htm`
...
grep -r "<!DOCTYPE" docs/htm --include="*.htm" --include="*.html" -l | wc -l

# Count total occurrences
grep -r "<!DOCTYPE" docs/htm --include="*.htm" --include="*.html" -c

# List affected files for verification
grep -r "<!DOCTYPE" docs/htm --include="*.htm" --include="*.html" -l
```
...
- [ ] I have verified I'm working in the correct git branch: `task-011-add-doctype`
...
1. **Verify setup**: Confirm you're in branch `task-011-add-doctype` and tracking shows "IN PROGRESS"
...
## Success Criteria
- All instances of `<!DOCTYPE` are properly addressed
